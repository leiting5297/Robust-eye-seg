...
#         self.MyConv = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=1)
...
# #         print("depth:",depth)
#         hh=[14,14,64,14,14,64,14,14,64,14,14,64]
#         ww=[14,14,64,14,14,64,14,14,64,14,14,64]
...
#         print("depth:",depth)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        logger.debug(f'patch_out shape={patch_out.shape}, high_freq shape={high_freq.shape}')
...
class FFParser(nn.Module):
    def __init__(self, in_chans=3, out_chans=768, h=64, w=64):
        super().__init__()
        self.to_channels = nn.Conv2d(in_chans, out_chans, kernel_size=1)  # 调整通道数
        self.to_size = nn.AdaptiveAvgPool2d((h, w))  # 调整尺寸
    ...
    def forward(self, x, spatial_size=None):
        ...
        mask = torch.rand(1, 3, 1024, 1024, device=x.device)
        ...
        # 调整通道数和空间尺寸
        inv = self.to_channels(inv)
        inv = self.to_size(inv)

        return inv


# class FFParser(nn.Module):
#     def __init__(self, dim=3, h=1024, w=1024):
#         super().__init__()
...
#         mask=torch.rand(1,3,1024,1024).to(x.device)
...
#         self.MLP_Adapter = Adapter(dim, skip_connect=True)  # MLP-adapter, no skip connection
        self.Space_Adapter = Adapter(dim,h,w, skip_connect=True)  # with skip connection
        self.scale = scale
#         self.Depth_Adapter = Adapter(dim, skip_connect=True)  # no skip connection
#         self.kk_adapter=Adapter(dim, skip_connect=True)
...
#         print('x5',x.shape)
        ...
        #x = x + self.kk_adapter(self.mlp(xn)) + self.scale * self.MLP_Adapter(xn)
        # x = self.FRH(x)
...
if __name__ == '__main__':
    ...
    test_model=ImageEncoderViT(parser.parse_args())
    tensor_1 = torch.rand(2,3,1024,1024)#257=256+1
    result=test_model(tensor_1)
#     print(result.shape)