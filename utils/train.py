...
args.path_helper = set_log_dir('logs', "111")
logger = create_logger(args.path_helper['log_path'])
logger.info(args)
...
checkpoint_path = os.path.join(settings.CHECKPOINT_PATH, args.net, "333")
#use tensorboard
if not os.path.exists(settings.LOG_DIR):
    os.mkdir(settings.LOG_DIR)
writer = SummaryWriter(log_dir=os.path.join(
        settings.LOG_DIR, args.net, "222"))
...
for epoch in range(200):
    if args.mod == 'sam_adpt':
        ...
        loss = function.train_sam(args, net, optimizer, nice_train_loader, epoch, writer, 1)
        logger.info(f'Train loss: {loss}|| @ epoch {epoch}.')
        time_end = time.time()
        print('time_for_training ', time_end - time_start)

        net.eval()
        if epoch and epoch % 20 == 0 or epoch == 199:
            tol, (eiou, edice) = function.validation_sam(args, nice_test_loader, epoch, net, writer, 1)
            ...
            }, is_best, args.path_helper['ckpt_path'], filename="best_checkpoint"+str(epoch))
            else:
                is_best = False
writer.close()
